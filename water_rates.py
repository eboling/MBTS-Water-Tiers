...
def draw_axes(canvas, values, graph_rect):
    y_sampler = Sampler((0, values[len(values) - 1]), (0, graph_rect.height()))
    x_sampler = Sampler((0, len(values)), (0, graph_rect.width()))
    ...
    for xin in xrange(0, graph_rect.width(), x_step):
        ...
        canvas.create_line(x, y0, x, y0 + 8, fill="black")
    ...
    for yin in xrange(0, int(values[len(values) - 1]), y_step):
        ...
        canvas.create_line(x0 - 8, y, x0, y, fill="black")
        canvas.create_text(x0 - 8 - 75, y, text="{0}".format(int(y_val)), anchor = tk.NW)
        
def do_graph(canvas, values, xMargin, yMargin):
    canvas_width =  800
    canvas_height = 500
    graph_rect = ViewRect(canvas_width, canvas_height,
                              (xMargin + 75, yMargin), (canvas_width - xMargin, canvas_height - yMargin))
    #canvas_width = canvas.winfo_width();
    #canvas_height = canvas.winfo_height();
    values.sort()
    draw_axes(canvas, values, graph_rect)
    #graph_interior_width = canvas_width - xMargin * 2
    #graph_interior_height = canvas_height - yMargin * 2
    if graph_rect.width() > len(values):
        sampling_step = 1
    else:
        sampling_step = float(len(values)) / float(graph_rect.width())
    max_usage = values[len(values) - 1]
    index = 0.0;
    x0 = graph_rect.map_x(0)
    y0 = graph_rect.map_y(0)
    #max_x = graph_rect.offset_x(graph_rect.width())
    #max_y = graph_rect.offset_y(graph_rect.height())
    max_y = graph_rect.height()
    for x1 in xrange (graph_rect.map_x(0), graph_rect.map_x(graph_rect.width())):
        y = values[int(index)]
        y1 = graph_rect.map_y(int((float(y) / max_usage) * float(max_y)))
        canvas.create_line(x0, y0, x1, y1, fill="black")
        #print "({0}, {1}) - ({2}, {3})".format(x0, y0, x1, y1)
        x0 = x1
        y0 = y1
        index = index + sampling_step
